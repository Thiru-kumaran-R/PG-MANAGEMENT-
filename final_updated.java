...
class Owner extends User {
    ...
    public void showMenu() {
        ...
        System.out.println("7. View Tenant Payments");
...
class PaymentService {
    ...
    public static void viewAllPayments() {
        ...
        System.out.println("\n--- TENANT PAYMENTS ---");
        for(Payment p : payments) {
            System.out.println(p.tenantName +" paid ₹" + p.amount);
        }
    }
}
...
class KYCService {
    ...
    public static void submitKYC(String phone, String pan, String aadhaar) {
        if (!phone.matches("\\d{10}"))            { System.out.println("Phone Number should contain exactly 10 digits."); return; }
        if (!pan.matches("[A-Z]{5}[0-9]{4}[A-Z]")){ System.out.println("PAN format should be like ABCDE1234F");   return; }
        if (!aadhaar.matches("\\d{12}"))           { System.out.println("Aadhaar Number should contain exactly 12 digits."); return; }
